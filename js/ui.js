...
// Remitos CRUD
$("btn-new").addEventListener("click", () =&gt; openRemitoModal(null)); $("btn-new-empty").addEventListener("click", () => openRemitoModal(null));
$("btn-save").addEventListener("click", handleSaveRemito); $("btn-confirm-delete").addEventListener("click", handleConfirmDelete);

// Scan / Galería
$("btn-scan").addEventListener("click", () =&gt; openScanModal("camera")); $("btn-gallery").addEventListener("click", () => openScanModal("gallery"));
$("scan-camera").addEventListener("change", handleScanImageSelected); $("scan-gallery-input").addEventListener("change", handleScanImageSelected);
...
// Exportar
$("btn-export-pdf").addEventListener("click", () =&gt; exportToPDF(_remitos, currentContextName())); $("btn-export-excel").addEventListener("click", () => exportToExcel(_remitos, currentContextName()));
...
$("import-file-excel").addEventListener("change", handleImportExcel); $("import-file-pdf").addEventListener("change", handleImportPDF);

// Grupos
$("btn-create-group").addEventListener("click", () =&gt; openModal("modal-create-group")); $("btn-save-group").addEventListener("click", handleCreateGroup);
$("btn-back-group").addEventListener("click", backToGroupList); $("btn-invite-member").addEventListener("click", () => openModal("modal-invite"));
$("btn-save-invite").addEventListener("click", handleInviteMember); $("btn-view-members").addEventListener("click", openMembersModal);

// Configuración
$("btn-save-settings").addEventListener("click", handleSaveSettings); $("field-ai-provider").addEventListener("change", (e) => updateSettingsHint(e.target.value));
...
const btn = $("btn-google-login"); const err = $("login-error");
err.textContent = "";
btn.disabled = true;
...
return <tr&gt; &lt;td&gt;&lt;span class="cell-numero"&gt;${esc(r.numeroRemito) || "—"}</span></td>
<td class="cell-date"> {esc(r.chofer) || "—"}</td>
<td><span class="tag tag-from"> {esc(r.hasta) || "—"}</span></td>
<td class="cell-litros"> {r.id}" aria-label="Editar">✏️</button>
<button class="btn-row btn-row-delete" data-action="delete" data-id=" {esc(r.numeroRemito)}" aria-label="Eliminar">🗑️</button>
</div>
</td>
</tr>`;
}
...
ge(containerId).innerHTML = entries.map(([label, value]) => &lt;div class="chart-row"&gt; &lt;div class="chart-row-header"&gt; &lt;span class="chart-label" title="${esc(label)}">${esc(label)}&lt;/span&gt; &lt;span class="chart-value"&gt;${fmtNum.format(value)} L</span>
</div>
...
if (!_groups.length) {
container.innerHTML = <div class="state-view"&gt; &lt;span class="state-icon"&gt;👥&lt;/span&gt; &lt;p&gt;No pertenecés a ningún grupo todavía.&lt;/p&gt; &lt;p style="font-size:.85rem;color:var(--color-text-2)"&gt;Creá uno o pedí que te inviten.&lt;/p&gt; &lt;/div>;
return;
}
container.innerHTML = _groups.map((g) => &lt;div class="group-card"&gt; &lt;div class="group-card-info"&gt; &lt;div class="group-card-name"&gt;${esc(g.name)}</div>
<div class="group-card-meta"> {g.id}" data-group-name="${esc(g.name)}"&gt; Entrar → &lt;/button&gt; &lt;/div>).join("");
...
const btn = $("btn-save-group"); btn.disabled = true; btn.textContent = "Creando…"; try { const user = _Auth.currentUser(); const profile = await _DB.getUserProfile(user.uid).catch(() =&gt; null); await _DB.createGroup({ name, creatorUid: user.uid, creatorEmail: user.email, creatorAlias: profile?.alias || "", }); ge("field-group-name").value = ""; closeModal("modal-create-group"); showToast(Grupo "${name}" creado ✓, "success");
} catch (err) {
...
const btn = $("btn-save-invite"); btn.disabled = true; btn.textContent = "Invitando…"; try { const email = await _DB.resolveInvite(input); if (!email) { showToast("No se encontró ningún usuario con ese alias.", "error"); return; } await _DB.inviteMember(_currentGroup.id, email); ge("field-invite").value = ""; closeModal("modal-invite"); showToast(${email} invitado/a al grupo ✓, "success");
} catch (err) {
...
ge("members-list").innerHTML = members.map((email) => &lt;div class="member-row"&gt; &lt;span class="member-email"&gt;📧${esc(email)}</span>
latex
{email !== _Auth.currentUser()?.email ? `<button class="btn btn-ghost btn-xs member-remove-btn" data-email="

{esc(email)}">Quitar</button>: '&lt;span class="member-you"&gt;Vos&lt;/span&gt;'} &lt;/div>).join("");
...
const url = https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}; const resp = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [ { inline_data: { mime_type: mediaType, data: base64Data } }, { text: OCR_PROMPT }, ]}], generationConfig: { maxOutputTokens: 400, temperature: 0.1 }, }), }); if (!resp.ok) { const e = await resp.json().catch(() =&gt; ({})); throw new Error(e?.error?.message ??Gemini HTTP ${resp.status}); }
...
method: "POST",
headers: { "Content-Type": "application/json", "Authorization": Bearer ${apiKey}}, body: JSON.stringify({ model: "gpt-4o", max_tokens: 400, messages: [{ role: "user", content: [ { type: "image_url", image_url: { url:data:${mediaType};base64,${base64Data}, detail: "high" } }, { type: "text", text: OCR_PROMPT }, ]}]}), }); if (!resp.ok) { const e = await resp.json().catch(() =&gt; ({})); throw new Error(e?.error?.message ??OpenAI HTTP ${resp.status}); }
const data = await resp.json();
...
const m = str.match(/^(\d{2})/(\d{2})/(\d{4}) {m[3]}- {m[1]}`;
...
docPDF.text(``${APP.EXPORT_TITLE} —  {fmtDate.format(new Date())} | Total: ${filtered.length} remitos, 14, 23);
